...
class OLM_v2(object):
    ...
    def biophys(self):
        ...
        for sec in self.soma:
            sec.IA.gkAbar = 0.0165
        self.soma.insert('Ih')
        for sec in self.soma:
            sec.Ih.gkhbar = 0.00035*0.1
        self.soma.insert('Ksoma')
        for sec in self.soma:
            sec.Ksoma.gksoma = 0.0391*1.5
        self.soma.insert('Nasoma')
        for sec in self.soma:
            sec.Nasoma.gnasoma = 0.0107*1.2
            sec.Nasoma.gl = 1./Rm
            sec.Nasoma.el = -67.0
         
        self.dend1.insert('IA')
        for sec in self.dend1:
            sec.IA.gkAbar = 0.004*1.2
        self.dend1.insert('Kdend')
        for sec in self.dend1:
            sec.Kdend.gkdend = 20*0.023
        self.dend1.insert('Nadend')
        for sec in self.dend1:
            sec.Nadend.gnadend = 2.0*0.0117
            sec.Nadend.gl = 1./Rm
            sec.Nadend.el = -65.0
        ...
        for sec in self.axon:
            sec.Kaxon.gkaxon = 0.05104
        self.axon.insert('Naaxon')
        for sec in self.axon:
            sec.Naaxon.gnaaxon = 0.01712
            sec.Naaxon.gl = 1./Rm
            sec.Naaxon.el = -67.0
