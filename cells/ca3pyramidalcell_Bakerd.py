...
def getReducedBiophysics(cell):
    with open('CA3_Bakerd_marascoProp.pickle') as f:
        props = pickle.load(f)
    ...
    RA = 140
    #Rm = 62996.0
    Rm = 68916.084857589894 # New input resistance
    Cm = 0.72
    PPSpineAdj=1.0
    RCSpineAdj=2.0
...
def addReducedSynapses(cell):
    for syntype in cell.synGroups:
        ...
        # oriensDistal
        syn = createSyn(cell.synvars,cell.oriensDistal,0.5)
        cell.synGroups[syntype]['oriensDistal'].append(syn)
        
        # oriensProximal
        syn = createSyn(cell.synvars,cell.oriensProximal,0.5)
        ...
        # lucidum
        syn = createSyn(cell.synvars,cell.lucidum,0.5)
        cell.synGroups[syntype]['lucidum'].append(syn)
        
        # radiatum
        syn = createSyn(cell.synvars,cell.radiatum,0.5)
        cell.synGroups[syntype]['radiatum'].append(syn)
        
        # lacunosumMEC
        syn = createSyn(cell.synvars,cell.lacunosumMEC,0.5)
        cell.synGroups[syntype]['lacunosumMEC'].append(syn)
        
        # lacunosumLEC
        syn = createSyn(cell.synvars,cell.lacunosumLEC,0.5)
